...
public class DialogueData
{
    ...
    public DialogueData(string speaker, string line, DialoguePosition position) //생성자, 다이얼로그 출력중 변경될 데이터들
    {
        this.dialogue = line;
...
public class DialogueManager : MonoBehaviour//Singleton<DialogueManager>
{
    ...
    public void ShowDialogue(DialogueData[] lines, DialogueEffect effect, DialogueAdvanceType advanceType, bool canSkip)
    {
        ...
        curEffect = effect; //현재 이펙트
        curAdvanceType = advanceType; //진행 타입
    ...
    private IEnumerator TextFadeIn(string line, string speaker)  //페이드인 효과 코루틴
    {
    ...
    private IEnumerator TextTyping(string line) //타이핑 효과
    {
    ...
    private IEnumerator AutoNextSentence()  //자동으로 다음텍스트로 넘어감 (페이드아웃)
    {
        Color nameColor = nameText.color;

        Color color = currentText.color;
        float timer = 0f;
        while (timer < autoAdvanceDelay)  //페이드아웃으로 사라짐
        {
            timer += Time.deltaTime;
